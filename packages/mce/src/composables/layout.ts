...
export function makeLayoutItemProps() {
  return {
    name: {
      type: String,
    },
...
function generateLayers(layout: string[], positions: Map<string, Ref<Position>>, layoutSizes: Map<string, Ref<number | string>>, activeItems: Map<string, Ref<boolean>>): { id: string, layer: Layer }[] {
  ...
  for (const id of layout) {
    ...
    const layer = {
      ...previousLayer,
      [position.value]: Number.parseInt(String(previousLayer[position.value]), 10)
        + (active.value ? Number.parseInt(String(amount.value), 10) : 0),
    }
...
export function createLayout(props: { overlaps?: string[], fullHeight?: boolean }) {
  ...
  const computedOverlaps = computed(() => {
    ...
    for (const overlap of overlaps.filter(item => item.includes(':'))) {
      ...
      map.set(bottom, { position: topPosition.value, amount: Number.parseInt(String(topAmount.value), 10) })
      map.set(top, { position: bottomPosition.value, amount: -Number.parseInt(String(bottomAmount.value), 10) })
    }
