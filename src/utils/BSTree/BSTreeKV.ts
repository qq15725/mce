...
export default class BSTreeKV<K, V extends K> {
    ...
    searchClosest(element: K, closestCompareFn: util.IClosestCompareFunction<K>): V | undefined {
        const ret = this.searchClosestNode(this.root, element, closestCompareFn);
        if (ret === null) {
            return undefined;
        }
        return ret.element;
    ...
    outorderTraversal(callback: util.ILoopFunction<V>): void {
        this.outorderTraversalAux(this.root, callback, {
            stop: false
        });
    }
     ...
     * @private
     */
    private searchClosestNode(node: BSTreeNode<V> | null, element: K, closestCompareFn: util.IClosestCompareFunction<K>): BSTreeNode<V> | null {
        let closest: BSTreeNode<V> | null = null;
        while (node !== null) {
            if (closestCompareFn(element, node.element, closest?.element ?? null)) {
                closest = node;
            }
            if (this.compare(element, node.element) < 0) {
                node = node.leftCh;
            } else {
                node = node.rightCh;
            }
     ...
     * @private
     */
    private outorderTraversalAux(node: BSTreeNode<V> | null, callback: util.ILoopFunction<V>, signal: { stop: boolean; }): void {
        if (node === null || signal.stop) {
            return;
        }
        this.outorderTraversalAux(node.rightCh, callback, signal);
        if (signal.stop) {
            return;
        }
        signal.stop = callback(node.element) === false;
        ...
        this.outorderTraversalAux(node.leftCh, callback, signal);
    }
